...
public static class Methods
{
    ...
    public static async Task<string> SolveTextCaptcha(BotData data,
        [BlockParam("Question", "The description of the captcha to solve, e.g. What is 2+2?")]
        string question,
            ...
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36")
    {
        ...
        var response = await data.Providers.Captcha.SolveRecaptchaV2Async(siteKey, siteUrl, sData, enterprise,
            isInvisible,
    ...
    public static Task<string> SolveRecaptchaV2(BotData data, string siteKey, string siteUrl, bool isInvisible = false,
        bool useProxy = false,
        string userAgent =
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36")
    ...
    // For backwards compatibility
    public static Task<string> SolveRecaptchaV3(BotData data, string siteKey, string siteUrl, string action,
            ...
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36")
    {
        ...
        var response = await data.Providers.Captcha.SolveKeyCaptchaAsync(userId, sessionId, webServerSign1,
            webServerSign2, siteUrl,
            SetupProxy(data, useProxy, userAgent), data.CancellationToken).ConfigureAwait(false);
    ...
    public static async Task ReportLastSolution(BotData data)
    {
        ...
        try
        {
            ...
            await data.Providers.Captcha
                .ReportSolution(long.Parse(lastCaptcha.Id), lastCaptcha.Type, false, data.CancellationToken)
                .ConfigureAwait(false);
            data.Logger.Log($"Solution of task {lastCaptcha.Id} reported correctly!", LogColors.ElectricBlue);
        }
        catch (Exception ex)
        {
            data.Logger.Log($"Could not report the solution of task {lastCaptcha.Id} to the service: {ex.Message}",
    ...
    private static async Task CheckBalance(BotData data)
    {
        ...
        {
            data.CaptchaCredit =
                await data.Providers.Captcha.GetBalanceAsync(data.CancellationToken).ConfigureAwait(false);
            data.Logger.Log($"[{data.Providers.Captcha.ServiceType}] Balance: ${data.CaptchaCredit}",
                LogColors.ElectricBlue);
