...
public static class Methods
{
        ...
        WaitUntilNavigation loadedEvent = WaitUntilNavigation.Load, string referer = "", int timeout = 30000)
    {
        ...
        var options = new NavigationOptions { Timeout = timeout, Referer = referer, WaitUntil = new[] { loadedEvent } };
        ...
        WaitUntilNavigation loadedEvent = WaitUntilNavigation.Load, int timeout = 30000)
    {
        ...
        var page = GetPage(data);
        var options = new NavigationOptions { Timeout = timeout, WaitUntil = new[] { loadedEvent } };
    ...
    [Block("Presses and releases a key in the browser page", name = "Key Press in Page",
        extraInfo =
            "Full list of keys here: https://github.com/puppeteer/puppeteer/blob/v1.14.0/lib/USKeyboardLayout.js")]
    ...
    public static async Task PuppeteerScreenshotPage(BotData data, string file, bool fullPage = false,
        bool omitBackground = false)
    {
        ...
        data.Logger.Log($"Took a screenshot of the {(fullPage ? "full" : "visible")} page and saved it to {file}",
            LogColors.DarkSalmon);
    }

    [Block("Takes a screenshot of the entire browser page and converts it to a base64 string",
        name = "Screenshot Page Base64")]
    public static async Task<string> PuppeteerScreenshotPageBase64(BotData data, bool fullPage = false,
        bool omitBackground = false)
    {
        data.Logger.LogHeader();
        ...
        data.Logger.Log($"Took a screenshot of the {(fullPage ? "full" : "visible")} page as base64",
            LogColors.DarkSalmon);
        return base64;
    }
    ...
    public static async Task PuppeteerSetViewport(BotData data, int width, int height, bool isMobile = false,
        bool isLandscape = false, float scaleFactor = 1f)
    {
        data.Logger.LogHeader();
    ...
    public static async Task<Dictionary<string, string>> PuppeteerGetCookies(BotData data, string domain)
    {
        ...
        data.Logger.Log(
            $"Got {cookies.Length} cookies for {(string.IsNullOrWhiteSpace(domain) ? "all domains" : domain)}",
            LogColors.DarkSalmon);
    ...
    public static async Task PuppeteerSetCookies(BotData data, string domain, Dictionary<string, string> cookies)
    {
        ...
        await page.SetCookieAsync(cookies
            .Select(c => new CookieParam { Domain = domain, Name = c.Key, Value = c.Value }).ToArray());
    ...
    public static async Task PuppeteerWaitForResponse(BotData data, string url, int timeoutMilliseconds = 60000)
    {
        ...
        var page = GetPage(data);
        var options = new WaitForOptions { Timeout = timeoutMilliseconds };
