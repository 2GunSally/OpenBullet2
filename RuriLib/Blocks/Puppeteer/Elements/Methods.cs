...
public static class Methods
{
    ...
    public static async Task PuppeteerSetAttributeValue(BotData data, FindElementBy findBy, string identifier,
        int index,
        ...
        string text)
    {
        ...
        foreach (var c in text)
        {
            ...
            await Task.Delay(data.Random.Next(100,
                300)); // Wait between 100 and 300 ms (average human type speed is 60 WPM ~ 360 CPM)
        }
        ...
        int timeBetweenClicks = 0)
    {
        ...
        var frame = GetFrame(data);
        var elem = await GetElement(frame, findBy, identifier, index);
        await elem.ClickAsync(new ClickOptions {
            Button = mouseButton, ClickCount = clickCount, Delay = timeBetweenClicks
        });
    ...
    public static async Task PuppeteerSelect(BotData data, FindElementBy findBy, string identifier, int index,
        string value)
    {
        data.Logger.LogHeader();
    ...
    public static async Task PuppeteerSelectByIndex(BotData data, FindElementBy findBy, string identifier, int index,
        int selectionIndex)
    {
        data.Logger.LogHeader();
    ...
    public static async Task PuppeteerSelectByText(BotData data, FindElementBy findBy, string identifier, int index,
        string text)
    {
        data.Logger.LogHeader();
        ...
        var elemScript = GetElementScript(findBy, identifier, index);
        var script =
            $"el={elemScript};for(let i=0;i<el.options.length;i++){{if(el.options[i].text=='{text}'){{el.selectedIndex = i;break;}}}}";
        await frame.EvaluateExpressionAsync(script);
    ...
    public static async Task<string> PuppeteerGetAttributeValue(BotData data, FindElementBy findBy, string identifier,
        int index,
    ...
    public static async Task<List<string>> PuppeteerGetAttributeValueAll(BotData data, FindElementBy findBy,
        string identifier,
        string attributeName = "innerText")
    {
        data.Logger.LogHeader();
        ...
        data.Logger.Log($"Got {values.Length} values for attribute {attributeName} by executing {script}",
            LogColors.DarkSalmon);
        return values.ToList();
    }

    [Block("Checks if an element is currently being displayed on the page", name = "Is Displayed")]
    public static async Task<bool> PuppeteerIsDisplayed(BotData data, FindElementBy findBy, string identifier,
        int index)
    {
        data.Logger.LogHeader();
        ...
        data.Logger.Log($"Found out the element is{(displayed ? "" : " not")} displayed by executing {script}",
            LogColors.DarkSalmon);
        return displayed;
    }
    ...
    public static async Task PuppeteerUploadFiles(BotData data, FindElementBy findBy, string identifier, int index,
    ...
    public static async Task<int> PuppeteerGetPositionX(BotData data, FindElementBy findBy, string identifier,
        int index)
    {
        data.Logger.LogHeader();
    ...
    public static async Task<int> PuppeteerGetPositionY(BotData data, FindElementBy findBy, string identifier,
        int index)
    {
        data.Logger.LogHeader();
    ...
    public static async Task PuppeteerScreenshotElement(BotData data, FindElementBy findBy, string identifier,
        int index,
        string fileName, bool fullPage = false, bool omitBackground = false)
    {
        ...
        var frame = GetFrame(data);
        var elem = await GetElement(frame, findBy, identifier, index);
        await elem.ScreenshotAsync(fileName,
    ...
    public static async Task<string> PuppeteerScreenshotBase64(BotData data, FindElementBy findBy, string identifier,
        int index,
    ...
    public static async Task PuppeteerWaitForElement(BotData data, FindElementBy findBy, string identifier,
        bool hidden = false, bool visible = true,
        int timeout = 30000)
    {
        data.Logger.LogHeader();
    ...
    private static async Task<IElementHandle> GetElement(IFrame frame, FindElementBy findBy, string identifier,
        int index)
    {
    ...
    private static string GetElementsScript(FindElementBy findBy, string identifier)
    {
        ...
        {
            var script =
                $"document.evaluate(\"{identifier.Replace("\"", "\\\"")}\", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null)";
