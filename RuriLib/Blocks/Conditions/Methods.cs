...
public static class Methods
{
    ...
    public static bool CheckCondition(BotData data, List<string> leftTerm, ListComparison comparison, string rightTerm)
    {
        ...
        if (result)
        {
            data.Logger.LogHeader();
            data.Logger.Log(
                $"Matched key {leftTerm.AsString().TruncatePretty(50)} {comparison} {rightTerm.TruncatePretty(50)}");
        }
    ...
    public static bool CheckCondition(BotData data, Dictionary<string, string> leftTerm, DictComparison comparison,
        string rightTerm)
    {
        var result = RuriLib.Functions.Conditions.Conditions.Check(leftTerm, comparison, rightTerm);
