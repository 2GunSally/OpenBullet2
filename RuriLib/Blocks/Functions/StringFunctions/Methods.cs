...
public static class Methods
{
        ...
        bool replaceOne = false)
    {
        ...
        data.Logger.LogHeader();
        data.Logger.Log($"Translated {replacements} occurrence(s). Translated string: {translated}",
    ...
    [Block("Generates a random string given a mask",
        extraInfo =
            "?l = Lowercase, ?u = Uppercase, ?d = Digit, ?f = Uppercase + Lowercase, ?s = Symbol, ?h = Hex (Lowercase), ?H = Hex (Uppercase), ?m = Upper + Digits,?n = Lower + Digits ?i = Lower + Upper + Digits, ?a = Any, ?c = Custom")]
