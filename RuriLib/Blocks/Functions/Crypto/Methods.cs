...
public static class Methods
{
    ...
    public static string HmacString(BotData data, string input, byte[] key,
        HashFunction hashFunction = HashFunction.MD5)
    {
    ...
    public static string ScryptString(BotData data, string password, string salt, int iterationCount = 16384,
        int blockSize = 8, int threadCount = 1)
    {
    ...
    {
        var derived =
            RuriLib.Functions.Crypto.Crypto.PBKDF2PKCS5(password, salt, saltSize, iterations, keyLength, type);
        data.Logger.LogHeader();
    ...
    [Block(
        "Generates a JSON Web Token using a secret key, payload, optional extra headers and specified algorithm type",
        name = "JWT Encode",
        extraInfo = "The header already contains the selected algorithm and token type (JWT) by default")]
    public static string JwtEncode(BotData data, JwtAlgorithmName algorithm, string secret, string extraHeaders = "{}",
        string payload = "{}")
    {
        ...
        var encoded =
            RuriLib.Functions.Crypto.Crypto.JwtEncode(algorithm, secret, extraHeadersDictionary, payloadDictionary);
    ...
    public static string BCryptHashGenSalt(BotData data, string input, int rounds = 10,
        SaltRevision saltRevision = SaltRevision.Revision2X)
    {
        data.Logger.LogHeader();
    ...
    private class FakeRNG : RandomNumberGenerator
    {
        ...
        {
            for (var i = 0; i < salt.Length; i++) data[i] = salt[i];
        }
    }
}