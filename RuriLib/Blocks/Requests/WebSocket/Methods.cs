...
public static class Methods
{
        ...
        Dictionary<string, string> customHeaders = null)
    {
        ...
        var factory = new Func<ClientWebSocket>(() =>
        {
            var client = new ClientWebSocket {
                Options = { KeepAliveInterval = TimeSpan.FromMilliseconds(keepAliveMilliseconds), Proxy = proxy }
            };
        ...
        var ws = new WebsocketClient(new Uri(url), factory) {
            IsReconnectionEnabled = false, ErrorReconnectTimeout = null
        };
        ...
        if (!ws.IsRunning) throw new Exception("Failed to connect to the websocket");
    ...
    public static async Task<List<string>> WsRead(BotData data, int pollIntervalInMilliseconds = 10,
    ...
    private static WebsocketClient GetSocket(BotData data)
        => data.TryGetObject<WebsocketClient>("webSocket") ??
    ...
    private static List<string> GetMessages(BotData data)
        => data.TryGetObject<List<string>>("wsMessages") ??
           throw new NullReferenceException("You must open a websocket connection first");
}