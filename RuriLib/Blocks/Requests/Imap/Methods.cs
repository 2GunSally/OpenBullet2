...
{
    private static readonly List<string> subdomains = new() {
        "mail",
        "imap-mail",
        "inbound",
        "in",
    ...
    public static async Task ImapAutoConnect(BotData data, string email, int timeoutMilliseconds = 60000)
    {
        ...
        var client = new ImapClient(protocolLogger) {
            Timeout = timeoutMilliseconds, ServerCertificateValidationCallback = (s, c, h, e) => true
        };

        if (data.UseProxy && data.Proxy != null) client.ProxyClient = MapProxyClient(data);
    ...
    private static async Task<bool> TryConnect(BotData data, ImapClient client, string domain, HostEntry entry)
    {
        ...
        try
        {
            await client.ConnectAsync(entry.Host, entry.Port, SecureSocketOptions.Auto, data.CancellationToken)
                .ConfigureAwait(false);
    ...
    {
        using var httpClient = HttpFactory.GetRLHttpClient(data.Proxy,
            new HttpOptions {
                ConnectTimeout = TimeSpan.FromMilliseconds(30000), ReadWriteTimeout = TimeSpan.FromMilliseconds(30000)
            });
    ...
    public static async Task<List<string>> ImapSearchMails(BotData data, SearchField field1 = SearchField.Subject,
        string text1 = "",
        SearchField field2 = SearchField.From, string text2 = "", int deliveredAfter = 1)
    {
        ...
        if (!string.IsNullOrEmpty(text1)) query = query.And(new TextSearchQuery(MapSearchTerm(field1), text1));

        if (!string.IsNullOrEmpty(text2)) query = query.And(new TextSearchQuery(MapSearchTerm(field2), text2));
    ...
    public static async Task<string> ImapReadMail(BotData data, string id, bool preferHtml = false)
    {
        ...
        if (string.IsNullOrEmpty(body) || preferHtml) body = mail.HtmlBody;
...
{body}";
    ...
    private static ImapClient GetAuthenticatedClient(BotData data)
    {
        ...
        if (!client.IsAuthenticated) throw new Exception("Authenticate the IMAP client first!");
    ...
    private static IMailFolder GetOpenInbox(BotData data)
    {
        ...
        if (!inbox.IsOpen) throw new Exception("Open the inbox first!");
