...
public static class Methods
{
        ...
        string username = "", string password = "", int timeoutMilliseconds = 10000)
    {
        ...
        if (!client.IsConnected) throw new Exception("Failed to connect to the FTP server with the given credentials");

        data.Logger.Log($"Connected to {host}:{port}", LogColors.Maize);
    }

    [Block("Lists the folders on the FTP server")]
    public static async Task<List<string>> FtpListItems(BotData data, FtpItemKind kind = FtpItemKind.FilesAndFolders,
