...
public static class Methods
{
    ...
    public static async Task TcpConnect(BotData data, string host, int port, bool useSSL,
        int timeoutMilliseconds = 10000)
    {
        data.Logger.LogHeader();

        var tcpClient = await TcpClientFactory.GetClientAsync(host, port,
                TimeSpan.FromMilliseconds(timeoutMilliseconds), data.UseProxy ? data.Proxy : null,
        ...
        if (useSSL)
        {
            ...
            await sslStream
                .AuthenticateAsClientAsync(new SslClientAuthenticationOptions { TargetHost = host },
    ...
    public static async Task<string> TcpSendRead(BotData data, string message, bool unescape = true,
        bool terminateWithCRLF = true,
    ...
    public static async Task TcpSend(BotData data, string message, bool unescape = true, bool terminateWithCRLF = true,
        [BlockParam("Use UTF8", "Enable to use UTF-8 encoding")]
        bool useUTF8 = false)
    {
        data.Logger.LogHeader();
    ...
    private static Stream GetStream(BotData data)
    {
        ...
        if (sslStream is not null) return sslStream;
