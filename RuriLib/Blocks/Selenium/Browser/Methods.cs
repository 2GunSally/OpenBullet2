...
public static class Methods
{
    ...
    public static void SeleniumOpenBrowser(BotData data, string extraCmdLineArgs = "")
    {
        ...
        {
            data.Logger.Log("The browser is already open, close it if you want to open a new browser",
                LogColors.JuneBud);
            return;
        }
        ...
        switch (provider.BrowserType)
        {
            case SeleniumBrowserType.Chrome:
                ...
                if (Utils.IsDocker())
                {
                    if (RootChecker.IsRoot()) chromeop.AddArgument("--no-sandbox");
                ...
                if (!string.IsNullOrWhiteSpace(args))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(extraCmdLineArgs)) args += ' ' + extraCmdLineArgs;
            ...
            case SeleniumBrowserType.Firefox:
                ...
                if (Utils.IsDocker()) fireop.AddArgument("--whitelisted-ips=''");
        ...
        data.Logger.Log(
            $"{(data.ConfigSettings.BrowserSettings.Headless ? "Headless " : "")}Browser opened successfully!",
            LogColors.JuneBud);
