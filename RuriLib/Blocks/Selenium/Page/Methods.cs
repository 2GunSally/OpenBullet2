...
public static class Methods
{
    ...
    [Block("Presses and releases a key in the browser page", name = "Key Press in Page",
        extraInfo =
            "Full list of keys here: https://github.com/SeleniumHQ/selenium/blob/master/dotnet/src/webdriver/Keys.cs")]
    ...
    private static string GetKeyCode(string key)
    {
        ...
        var matchingField =
            keyFields.FirstOrDefault(f => f.Name.Equals(key, StringComparison.InvariantCultureIgnoreCase));

        if (matchingField == null) throw new Exception($"Invalid key name: {key}");
