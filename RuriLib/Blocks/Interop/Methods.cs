...
public static class Methods
{
    ...
    {
        data.Logger.LogHeader();
        var result = await StaticNodeJSService
            .InvokeFromFileAsync<T>(scriptFile, null, parameters, data.CancellationToken).ConfigureAwait(false);
    ...
    public static Engine InvokeJint(BotData data, Engine engine, string scriptFile)
    {
        ...
        data.Logger.Log($"Executed Jint script with completion value: {engine.GetCompletionValue()}",
