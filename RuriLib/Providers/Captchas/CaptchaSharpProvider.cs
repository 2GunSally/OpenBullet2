...
public class CaptchaSharpProvider : ICaptchaProvider
{
    ...
    public Task ReportSolution(long id, CaptchaType type, bool correct = false,
        CancellationToken cancellationToken = default)
        => service.ReportSolution(id, type, correct, cancellationToken);

    public Task<CapyResponse> SolveCapyAsync(string siteKey, string siteUrl, Proxy proxy = null,
        CancellationToken cancellationToken = default)
    ...
    public Task<StringResponse> SolveHCaptchaAsync(string siteKey, string siteUrl, Proxy proxy = null,
        CancellationToken cancellationToken = default)
        => service.SolveHCaptchaAsync(siteKey, siteUrl, proxy, cancellationToken);

    public Task<StringResponse> SolveImageCaptchaAsync(string base64, ImageCaptchaOptions options = null,
        CancellationToken cancellationToken = default)
    ...
    public Task<StringResponse> SolveKeyCaptchaAsync(string userId, string sessionId, string webServerSign1,
        ...
        => service.SolveKeyCaptchaAsync(userId, sessionId, webServerSign1, webServerSign2, siteUrl, proxy,
            cancellationToken);
    ...
    public Task<StringResponse> SolveTextCaptchaAsync(string text, TextCaptchaOptions options = null,
        CancellationToken cancellationToken = default)
