...
public class FileEmailDomainRepository : IEmailDomainRepository
{
    ...
    public Task<IEnumerable<HostEntry>> GetImapServers(string domain)
        => Task.FromResult(
            (imapHosts.ContainsKey(domain) ? imapHosts[domain] : new List<HostEntry>()) as IEnumerable<HostEntry>);

    public Task<IEnumerable<HostEntry>> GetPop3Servers(string domain)
        => Task.FromResult(
            (pop3Hosts.ContainsKey(domain) ? pop3Hosts[domain] : new List<HostEntry>()) as IEnumerable<HostEntry>);

    public Task<IEnumerable<HostEntry>> GetSmtpServers(string domain)
        => Task.FromResult(
            (smtpHosts.ContainsKey(domain) ? smtpHosts[domain] : new List<HostEntry>()) as IEnumerable<HostEntry>);
    ...
    private static Task TryAddServer(string file, ConcurrentDictionary<string, List<HostEntry>> hosts, string domain,
        HostEntry server)
    {
