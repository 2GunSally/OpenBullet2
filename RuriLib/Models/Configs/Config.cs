...
public class Config
{
    ...
    [JsonIgnore] public List<(BlockInstance, int)> DeletedBlocksHistory { get; set; } = new();
    ...
    public void ChangeMode(ConfigMode newMode)
    {
        ...
        var mappings = new Dictionary<(ConfigMode, ConfigMode), Action> {
            { (ConfigMode.Stack, ConfigMode.LoliCode), () => LoliCodeScript = Stack2LoliTranspiler.Transpile(Stack) }, {
                (ConfigMode.Stack, ConfigMode.CSharp),
            ...
            { (ConfigMode.LoliCode, ConfigMode.Stack), () => Stack = Loli2StackTranspiler.Transpile(LoliCodeScript) }, {
                (ConfigMode.LoliCode, ConfigMode.CSharp),
    ...
    public bool HasUnsavedChanges()
        => Mode switch {
            ConfigMode.Stack => GetHash(JsonConvert.SerializeObject(Stack) + JsonConvert.SerializeObject(Settings)) !=
                                stackerHash,
