...
public class MultiRunJob : Job
{
    ...
    private readonly Func<MultiRunInput, CancellationToken, Task<CheckResult>> workFunction =
        async (input, token) =>
        {
            ...
            if (!botData.Line.IsValid || !botData.Line.RespectsRules(botData.ConfigSettings.DataSettings.DataRules))
            {
                ...
                // RETURN THE RESULT
                return new CheckResult { BotData = botData, OutputVariables = new Dictionary<string, object>() };
            }
            ...
            if (input.IsLegacy)
            {
                lsGlobals = new LSGlobals(botData) {
                    Globals = input.LegacyGlobals, GlobalCookies = input.LegacyGlobalCookies
                };
                ...
                foreach (var customInput in botData.ConfigSettings.InputSettings.CustomInputs)
                    legacyVariables.Set(
                        new StringVariable(customInput.DefaultAnswer) { Name = customInput.VariableName });
            ...
            try
            {
                ...
                if (botData.UseProxy)
                {
                    ...
                    if (botData.Proxy == null)
                    {
                        if (input.Job.NoValidProxyBehaviour == NoValidProxyBehaviour.Reload)
                            try
                            {
                                await input.BotData.AsyncLocker.Acquire(typeof(ProxyPool),
                                    nameof(ProxyPool.ReloadAllAsync),
                ...
                botData.Logger.Log(
                    $"[{DateTime.Now.ToLongTimeString()}] BOT STARTED WITH DATA {botData.Line.Data} AND PROXY {botData.Proxy}");

                // If it's a DLL config, invoke the method
                if (input.IsDLL)
                {
                    var task = (Task)input.DLLMethod.Invoke(null,
                        new object[] { botData, scriptGlobals.input, scriptGlobals.globals, outputVariables, token });
                ...
                else if (input.IsLegacy)
                {
                    ...
                    do
                    {
                        if (botData.CancellationToken.IsCancellationRequested) break;
            ...
            finally
            {
                ...
                if (botData.ConfigSettings.BrowserSettings.QuitBrowserStatuses.Contains(botData.STATUS))
                    ...
                    botData.DisposeObjectsExcept(new[] {
                        "puppeteer", "puppeteerPage", "puppeteerFrame", "httpClient", "ironPyEngine"
                    });
            }

            // Update captcha credit
            if (botData.CaptchaCredit > 0) input.Job.CaptchaCredit = botData.CaptchaCredit;
            ...
            if (botData.Proxy != null)
            {
                ...
                // Otherwise set it to available
                    ...
                    input.ProxyPool.ReleaseProxy(input.BotData.Proxy);
            }
                ...
                if (botData.ConfigSettings.GeneralSettings.ReportLastCaptchaOnRetry)
                {
                    ...
                    if (lastCaptcha is not null)
                        try
                        {
                            ...
                            await botData.Providers.Captcha
                                .ReportSolution(long.Parse(lastCaptcha.Id), lastCaptcha.Type, false, cts.Token)
            ...
            // RETURN THE RESULT
            return new CheckResult { BotData = botData, OutputVariables = outputVariables };
        };
    ...
    public List<ProxySource> ProxySources { get; set; } = new();
    ...
    public List<IHitOutput> HitOutputs { get; set; } = new();
    public Bots.Providers Providers { get; set; }
    public Dictionary<string, string> CustomInputsAnswers { get; set; } = new();
    ...
    public List<Hit> Hits { get; private set; } = new();
    ...
    public int ProxiesAlive => proxyPool == null
        ? 0
        : proxyPool.Proxies
            .Count(p => p.ProxyStatus == ProxyStatus.Available || p.ProxyStatus == ProxyStatus.Busy);

    public int ProxiesBanned =>
        proxyPool == null ? 0 : proxyPool.Proxies.Count(p => p.ProxyStatus == ProxyStatus.Banned);
    ...
    public async Task FetchProxiesFromSources(CancellationToken cancellationToken = default)
    {
        try
        {
            await asyncLocker.Acquire(typeof(ProxyPool), nameof(ProxyPool.ReloadAllAsync), cancellationToken)
                .ConfigureAwait(false);
    ...
    public async override Task Start(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            legacyGlobalVariables = new VariablesList();
            legacyGlobalCookies = new Dictionary<string, string>();

            if (wordlistType == null)
                throw new NullReferenceException(
                    $"The wordlist type with name {DataPool.WordlistType} was not found in the Environment");

            resources = new Dictionary<string, ConfigResource>();
            ...
            if (!string.IsNullOrWhiteSpace(Config.StartupCSharpScript))
            {
                ...
                var startupBotData =
                    new BotData(Providers, Config.Settings, new BotLogger(), new DataLine(string.Empty, wordlistType)) {
    ...
    public async override Task Stop()
    {
        try
        {
            if (parallelizer is not null) await parallelizer.Stop().ConfigureAwait(false);
        }
        finally
        {
            StopTimers();
    ...
    public async override Task Abort()
    {
        try
        {
            if (parallelizer is not null) await parallelizer.Abort().ConfigureAwait(false);
        }
        finally
        {
            StopTimers();
    ...
    public async override Task Pause()
    {
        try
        {
            if (parallelizer is not null) await parallelizer.Pause().ConfigureAwait(false);
        }
        finally
        {
            StopTimers();
    ...
    {
        if (parallelizer is not null) await parallelizer.Resume().ConfigureAwait(false);
    ...
    private void PropagateProgress(object _, float progress) => OnProgress?.Invoke(this, progress);
        ...
        if (PeriodicReloadInterval > TimeSpan.Zero)
            proxyReloadTimer = new Timer(async _ =>
            {
                ...
                if (proxyPool is not null)
                    try
                    {
                        await asyncLocker.Acquire(typeof(ProxyPool), nameof(ProxyPool.ReloadAllAsync),
    ...
    private void DataProcessed(object sender, ResultDetails<MultiRunInput, CheckResult> details)
    {
        ...
        {
            case "SUCCESS":
                Interlocked.Increment(ref dataHits);
                break;
            case "NONE":
                Interlocked.Increment(ref dataToCheck);
                break;
            case "FAIL":
                Interlocked.Increment(ref dataFails);
                break;
            case "INVALID":
                Interlocked.Increment(ref dataInvalid);
                break;
            default:
                Interlocked.Increment(ref dataCustom);
                break;
        }

        Interlocked.Increment(ref dataTested);

        if (parallelizer.Status == ParallelizerStatus.Stopping) details.Item.BotData.ExecutionInfo = "STOPPED";
    }
    ...
    private async Task RegisterHit(CheckResult result)
    {
        ...
        foreach (var hitOutput in HitOutputs) await hitOutput.Store(hit).ConfigureAwait(false);
    }
    ...
    {
        if (Providers.GeneralSettings.VerboseMode) Console.WriteLine($"[{DateTime.Now}] {message}");
    }
