...
public class LinesFromFileResource : ConfigResource, IDisposable
{
    ...
    {
        lock (streamLocker) return TakeOneUnsafe();
    }
    ...
    private string TakeOneUnsafe()
    {
        ...
        if (line is null)
        {
            ...
            if (linesRead == 0) throw new Exception($"Resource '{options.Name}' has no more valid lines to take");
        ...
        if (string.IsNullOrWhiteSpace(line) && options.IgnoreEmptyLines) return TakeOne();
    ...
    public override List<string> Take(int amount)
    {
        lock (streamLocker)
        {
            ...
            for (var i = 0; i < amount; i++) lines.Add(TakeOneUnsafe());
