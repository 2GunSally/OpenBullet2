...
public class RandomLinesFromFileResource : ConfigResource
{
    ...
    {
        lock (sourceLocker) return TakeOneUnsafe();
    }

    private string TakeOneUnsafe()
    {
        if (source.Count == 0) throw new Exception($"Resource '{options.Name}' has no more valid lines to take");

        var line = source[random.Next(source.Count)];

        if (options.Unique) source.Remove(line);
