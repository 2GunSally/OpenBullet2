...
public class DataLine
{
    ...
    public List<StringVariable> GetVariables()
    {
        ...
        var split = string.IsNullOrEmpty(Type.Separator)
            ? new[] { Data }
            : Data.Split(Type.Separator, Type.Slices.Length);
    ...
    public bool RespectsRules(IEnumerable<DataRule> rules)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            if (slice == null) throw new ArgumentException($"Invalid slice name ({rule.SliceName}) in a data rule");

            var value = slice.AsString();

            if (!rule.IsSatisfied(value)) return false;
        }
