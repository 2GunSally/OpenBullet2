...
public class ConfigDebugger
{
    ...
    public async Task Run()
    {
        ...
        if (config.Mode == ConfigMode.Stack || config.Mode == ConfigMode.LoliCode)
        {
            ...
            config.StartupCSharpScript =
                Loli2CSharpTranspiler.Transpile(config.StartupLoliCodeScript, config.Settings, options.StepByStep);
        }

        if (options.UseProxy && !options.TestProxy.Contains(':')) throw new InvalidProxyException(options.TestProxy);

        if (!options.PersistLog) logger.Clear();

        // Close any previously opened browsers
        if (lastPuppeteerBrowser != null) await lastPuppeteerBrowser.CloseAsync().ConfigureAwait(false);

        if (lastSeleniumBrowser != null) lastSeleniumBrowser.Quit();
        ...
        var providers = new Bots.Providers(RuriLibSettings) { RNG = RNGProvider };
        ...
        if (stepper != null) stepper.WaitingForStep -= OnWaitingForStep;
        ...
        data = new BotData(providers, config.Settings, logger, dataLine, proxy, options.UseProxy) {
            CancellationToken = cts.Token, Stepper = stepper
        };
        ...
        data.AsyncLocker = new AsyncLocker();
            ...
            if (config.Mode != ConfigMode.Legacy)
            {
                ...
                if (!string.IsNullOrWhiteSpace(config.StartupCSharpScript))
                {
                    ...
                    var startupData = new BotData(providers, config.Settings, logger,
                        new DataLine(string.Empty, wordlistType)) { CancellationToken = cts.Token, Stepper = stepper };
    ...
    {
        if (stepper == null || !stepper.IsWaiting) return false;
