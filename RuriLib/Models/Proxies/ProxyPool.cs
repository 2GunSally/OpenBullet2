...
public class ProxyPool : IDisposable
{
    ...
    {
        if (isReloadingProxies) return;

        try
        {
            isReloadingProxies = true;
            await asyncLocker.Acquire(typeof(ProxyPool), nameof(ReloadAllAsync), cancellationToken)
            ...
            while (currentTry < maxReloadTries)
            {
                ...
                if (await TryReloadAllAsync(shuffle)) return;

                // If it fails to fetch at least 1 proxy, backoff by an increasing amount (e.g. to prevent rate limiting)
                Console.WriteLine(
                    $"Failed to reload, no proxies found. Waiting {currentBackoff} ms and trying again...");
        ...
        finally
        {
            ...
            asyncLocker.Release(typeof(ProxyPool), nameof(ReloadAllAsync));
        }
    }
