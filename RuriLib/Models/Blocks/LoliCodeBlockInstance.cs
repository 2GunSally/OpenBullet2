...
public class LoliCodeBlockInstance : BlockInstance
{
    ...
    private string TranspileStatement(string input, List<string> definedVariables)
    {
        ...
        // CODE LABEL
        // #MYLABEL => MYLABEL:
        if ((match = Regex.Match(input, $"^#({validTokenRegex})$")).Success) return $"{match.Groups[1].Value}:";
        ...
        // ELSE
        // ELSE => } else {
        if (input == "ELSE") return $"}}{System.Environment.NewLine}else{System.Environment.NewLine}{{";
        ...
        if ((match = Regex.Match(input, "ELSE IF (.+)$")).Success)
        {
            ...
            if (LoliCodeParser.keyIdentifiers.Any(t => line.StartsWith(t)))
            {
                ...
                var key = LoliCodeParser.ParseKey(ref line, keyType);
                return
                    $"}}{System.Environment.NewLine}else if ({CSharpWriter.ConvertKey(key)}){System.Environment.NewLine}{{";
            }
        ...
        // TRY
        // TRY => try {
        if (input == "TRY") return $"try{System.Environment.NewLine}{{";

        // CATCH
        // CATCH => } catch {
        if (input == "CATCH") return $"}}{System.Environment.NewLine}catch{System.Environment.NewLine}{{";

        // FINALLY
        // FINALLY => } finally {
        if (input == "FINALLY") return $"}}{System.Environment.NewLine}finally{System.Environment.NewLine}{{";
        ...
        if ((match = Regex.Match(input, $"^SET CAP @?\"?({validTokenRegex})\"? (.+)$")).Success)
        {
            if (definedVariables.Contains(match.Groups[1].Value))
                return
                    $"{match.Groups[1].Value} = {match.Groups[2].Value};{System.Environment.NewLine}data.MarkForCapture(nameof({match.Groups[1].Value}));";
            definedVariables.Add(match.Groups[1].Value);
            return
                $"string {match.Groups[1].Value} = {match.Groups[2].Value};{System.Environment.NewLine}data.MarkForCapture(nameof({match.Groups[1].Value}));";
        }
