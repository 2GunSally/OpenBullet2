...
public class ScriptBlockInstance : BlockInstance
{
    ...
    public List<OutputVariable> OutputVariables { get; set; } = new() {
        new() { Name = "result", Type = VariableType.Int }
    };
    ...
    public override void FromLC(ref string script, ref int lineNumber)
    {
        ...
        catch
        {
            throw new LoliCodeParsingException(lineNumber,
                $"Invalid interpreter definition: {line.TruncatePretty(50)}");
        }
        ...
        switch (Interpreter)
        {
            case Interpreter.Jint:
                ...
                foreach (var output in OutputVariables)
                {
                    ...
                    writer.WriteLine(
                        $"{output.Name} = {engineName}.Global.GetProperty(\"{output.Name}\").Value.{GetJintMethod(output.Type)};");
                }
...
return noderesult;
                ...
                writer.WriteLine(
                    $"var {resultName} = await InvokeNode<dynamic>(data, \"{scriptPath}\", new object[] {{ {InputVariables} }});");
                ...
                foreach (var output in OutputVariables)
                {
                    ...
                    writer.WriteLine(
                        $"{output.Name} = {resultName}.GetProperty(\"{output.Name}\").{GetNodeMethod(output.Type)};");
                }
                ...
                foreach (var output in OutputVariables)
                {
                    ...
                    writer.WriteLine($"{output.Name} = {scopeName}" + output.Type switch {
                        VariableType.ListOfStrings =>
                            $".GetVariable<IList<object>>(\"{output.Name}\").Cast<string>().ToList();",
                        VariableType.ByteArray =>
                            $".GetVariable<IList<object>>(\"{output.Name}\").Cast<byte>().ToArray();",
        ...
        foreach (var output in OutputVariables) writer.WriteLine($"data.LogVariableAssignment(nameof({output.Name}));");
