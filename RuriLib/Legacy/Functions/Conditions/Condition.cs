...
public static class Condition
{
    ...
    public static bool ReplaceAndVerify(KeycheckCondition kcCond, LSGlobals ls)
    {
        var style = NumberStyles.Number |
                    NumberStyles.AllowCurrencySymbol; // Needed when comparing values with a currency symbol
        var provider = new CultureInfo("en-US");
        var L = BlockBase.ReplaceValuesRecursive(kcCond.Left,
            ls); // The left-hand term can accept recursive values like <LIST[*]>
        ...
        switch (kcCond.Comparer)
        {
            ...
            case Comparer.GreaterThan:
                return L.Any(l =>
                    decimal.Parse(l.Replace(',', '.'), style, provider) >
                    decimal.Parse(r.Replace(',', '.'), style, provider));
    ...
    public static bool Verify(KeycheckCondition kcCond)
    {
        ...
        switch (kcCond.Comparer)
        {
            ...
            case Comparer.GreaterThan:
                return decimal.Parse(kcCond.Left.Replace(',', '.'), style, provider) >
                       decimal.Parse(kcCond.Right.Replace(',', '.'), style, provider);
            ...
            case Comparer.DoesNotExist:
                throw new NotSupportedException(
                    "Exists and DoesNotExist operators are only supported in the ReplaceAndVerify method.");
