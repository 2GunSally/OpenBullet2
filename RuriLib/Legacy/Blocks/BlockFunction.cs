...
public class BlockFunction : BlockBase
{
    ...
    public async override Task Process(LSGlobals ls)
    {
        ...
        for (var i = 0; i < localInputStrings.Count; i++)
        {
            ...
            switch (FunctionType)
            {
                ...
                case Function.HMAC:
                    outputString = Hmac(localInputString, HashType, ReplaceValues(HmacKey, ls), InputBase64, KeyBase64,
                        HmacBase64);
                    break;
                ...
                case Function.Replace:
                    outputString = UseRegex
                        ? Regex.Replace(localInputString, ReplaceValues(ReplaceWhat, ls),
                            ReplaceValues(ReplaceWith, ls))
                ...
                case Function.RandomNum:
                    ...
                    outputString = RandomZeroPad
                        ? randomNumString.PadLeft(max.ToString().Length, '0')
                        : randomNumString;
                    break;

                case Function.RandomString:
                    outputString = localInputString;
                    outputString = Regex.Replace(outputString, @"\?l",
                        m => _lowercase[data.Random.Next(_lowercase.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?u",
                        m => _uppercase[data.Random.Next(_uppercase.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?d",
                        m => _digits[data.Random.Next(_digits.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?s",
                        m => _symbols[data.Random.Next(_symbols.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?h",
                        m => _hex[data.Random.Next(_hex.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?a",
                        m => _allChars[data.Random.Next(_allChars.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?m",
                        m => _udChars[data.Random.Next(_udChars.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?n",
                        m => _ldChars[data.Random.Next(_ldChars.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?i",
                        m => _ludChars[data.Random.Next(_ludChars.Length)].ToString());
                    outputString = Regex.Replace(outputString, @"\?f",
                        m => _upperlwr[data.Random.Next(_upperlwr.Length)].ToString());
                    break;
                ...
                case Function.Round:
                    outputString = Math.Round(decimal.Parse(localInputString, _style, _provider), 0,
            ...
            case Function.RSADecrypt:
                ...
                */
                ...
                case Function.Delay:
                    try { Thread.Sleep(int.Parse(localInputString)); }
                    catch { }
                ...
                case Function.Substring:
                    outputString = localInputString.Substring(int.Parse(ReplaceValues(SubstringIndex, ls)),
                        int.Parse(ReplaceValues(SubstringLength, ls)));
                    break;
            ...
            data.Logger.Log($"Executed function {FunctionType} on input {localInputString} with outcome {outputString}",
                LogColors.GreenYellow);

            // Add to the outputs
            outputs.Add(outputString);
        }

        var isList = outputs.Count > 1 || InputString.Contains("[*]") || InputString.Contains("(*)") ||
                     InputString.Contains("{*}");
        InsertVariable(ls, IsCapture, isList, outputs, VariableName);
    }
    ...
    ///     Whether the base string should be treated as base64 encoded (if false, it will be treated as
    ///     UTF8 encoded)
    /// </param>
    ...
    /// <param name="inputBase64">
    ///     Whether the input string should be treated as base64 encoded (if false, it will be treated as
    ///     UTF8 encoded)
    /// </param>
    /// <param name="keyBase64">
    ///     Whether the key string should be treated as base64 encoded (if false, it will be treated as
    ///     UTF8 encoded)
    /// </param>
    ...
    public static string Hmac(string baseString, Hash type, string key, bool inputBase64, bool keyBase64,
        bool outputBase64)
    {
        var rawInput = inputBase64 ? Convert.FromBase64String(baseString) : Encoding.UTF8.GetBytes(baseString);
    ...
    public bool UserAgentSpecifyBrowser { get; set; }
