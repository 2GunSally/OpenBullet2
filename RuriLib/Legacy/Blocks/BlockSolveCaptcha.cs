...
public class BlockSolveCaptcha : BlockBase
{
    ...
    public override BlockBase FromLS(string line)
    {
        ...
        switch (Type)
        {
            case CaptchaType.TextCaptcha:
                ...
                LanguageGroup =
                    (CaptchaLanguageGroup)LineParser.ParseEnum(ref input, "LANG GROUP", typeof(CaptchaLanguageGroup));
    ...
    public async override Task Process(LSGlobals ls)
    {
        ...
        Proxy proxy = data.UseProxy && UseProxy
            ? proxy = new Proxy {
                ...
                UserAgent = UserAgent,
                Cookies = data.COOKIES.ToList().Concat(ls.GlobalCookies.ToList()).Select(p => (p.Key, p.Value))
                    .ToArray()
            }
            : null;
        ...
        if (provider.CheckBalanceBeforeSolving)
            try
            {
                try
                {
                    ...
                    if (data.CaptchaCredit < (decimal)0.002) throw new Exception("The remaining balance is too low!");
                }
            ...
            try
            {
                ...
                switch (response)
                {
                    case StringResponse r:
                        ...
                        data.Logger.Log($"Captcha solved successfully! Id: {r.IdString} Solution: {r.Response}",
                    ...
                    case GeeTestResponse r:
                        ...
                        data.Logger.Log(
                            $"Captcha solved successfully! Id: {r.IdString} Challenge: {r.Challenge}\r\nValidate: {r.Validate}\r\nSecCode: {r.SecCode}",
                            LogColors.GreenYellow);
                        break;
                }
        ...
        {
            errorMessage =
                $"The currently selected service ({provider.ServiceType}) does not support this task! {ex.Message}";
        }
        ...
        if (!string.IsNullOrEmpty(errorMessage)) data.Logger.Log(errorMessage, LogColors.Tomato);
    }
    ...
    private async Task<CaptchaResponse> GetResponse(LSGlobals ls, Proxy proxy)
    {
        ...
        return Type switch {
            CaptchaType.TextCaptcha => await provider.SolveTextCaptchaAsync(ReplaceValues(Question, ls),
                new TextCaptchaOptions { CaptchaLanguage = Language, CaptchaLanguageGroup = LanguageGroup }),

            CaptchaType.ImageCaptcha => await provider.SolveImageCaptchaAsync(ReplaceValues(Base64, ls),
            ...
            CaptchaType.ReCaptchaV2 => await provider.SolveRecaptchaV2Async(ReplaceValues(SiteKey, ls),
                ReplaceValues(SiteUrl, ls),
                "", false, IsInvisible, proxy),

            CaptchaType.ReCaptchaV3 => await provider.SolveRecaptchaV3Async(ReplaceValues(SiteKey, ls),
                ReplaceValues(SiteUrl, ls),
                ReplaceValues(Action, ls), float.Parse(ReplaceValues(MinScore, ls)), false, proxy),

            CaptchaType.FunCaptcha => await provider.SolveFuncaptchaAsync(ReplaceValues(PublicKey, ls),
                ReplaceValues(ServiceUrl, ls),
                ReplaceValues(SiteUrl, ls), NoJS, proxy),

            CaptchaType.HCaptcha => await provider.SolveHCaptchaAsync(ReplaceValues(SiteKey, ls),
                ReplaceValues(SiteUrl, ls), proxy),

            CaptchaType.Capy => await provider.SolveCapyAsync(ReplaceValues(SiteKey, ls), ReplaceValues(SiteUrl, ls),
                proxy),

            CaptchaType.KeyCaptcha => await provider.SolveKeyCaptchaAsync(ReplaceValues(UserId, ls),
                ReplaceValues(SessionId, ls),
                ReplaceValues(WebServerSign1, ls), ReplaceValues(WebServerSign2, ls), ReplaceValues(SiteUrl, ls),
                proxy),
