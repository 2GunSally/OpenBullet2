...
public class BlockParse : BlockBase
{
    ...
    public async override Task Process(LSGlobals ls)
    {
        ...
        var list = Type switch {
            ParseType.LR => ParseLR(original, ReplaceValues(LeftString, ls), ReplaceValues(RightString, ls),
                UseRegexLR),
            ...
            ParseType.REGEX => ParseREGEX(original, ReplaceValues(RegexString, ls), ReplaceValues(RegexOutput, ls),
                DotMatches, CaseSensitive),
            _ => throw new NotImplementedException()
        };

        if (!Recursive && Type == ParseType.CSS) list = new List<string> { list[CssElementIndex] };
    ...
    private static List<string> ParseLR(string original, string left, string right, bool useRegex)
    {
        ...
        else
        {
            ...
            foreach (Match match in Regex.Matches(original, pattern)) list.Add(match.Value);
        }
    ...
    {
        if (jTokenParsing) return JsonParser.GetValuesByKey(original, fieldName).ToList();
    ...
    private static List<string> ParseREGEX(string original, string pattern, string format, bool multiLine,
        bool caseSensitive)
    {
    ...
    /// <summary>
    ///     The way the content of the matched groups should be formatted. [0] will be replaced with the full match, [1]
    ///     with the first group etc.
    /// </summary>
