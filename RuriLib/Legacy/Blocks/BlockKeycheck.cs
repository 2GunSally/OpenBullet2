...
public class BlockKeycheck : BlockBase
{
    ...
    public override BlockBase FromLS(string line)
    {
        ...
        while (input != string.Empty) // Scan for keychains until the end
        {
            ...
            var kc = new KeyChain {
                Type = (KeychainType)LineParser.ParseEnum(ref input, "Keychain Type", typeof(KeychainType))
            };
            ...
            while (!input.StartsWith("KEYCHAIN") && input != string.Empty) // Scan for keys
            {
                ...
                if (LineParser.CheckIdentifier(ref input, "KEY") || LineParser.CheckIdentifier(ref input, "KEYCHAIN") ||
                    input == string.Empty)
                {
                    k.LeftTerm = "<SOURCE>";
    ...
    public async override Task Process(LSGlobals ls)
    {
        ...
        // Check Global Keys
        if (data.Providers.ProxySettings.ContainsBanKey(data.SOURCE, out var _))
        {
            data.STATUS = "BAN";
            return;
        }
        ...
        // Check Retry Keys
        if (data.Providers.ProxySettings.ContainsRetryKey(data.SOURCE, out var _))
        {
            data.STATUS = "RETRY";
            return;
        }
        ...
        if (!found && BanOnToCheck) data.STATUS = "BAN";
    }
}