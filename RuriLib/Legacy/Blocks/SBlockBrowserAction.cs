...
public class SBlockBrowserAction : BlockBase
{
    ...
    public enum BrowserAction
    {
        ...
        /// <summary>
        ///     Sends some keystrokes to the browser. Special keys like &lt;ENTER&gt; can be sent, separated by two pipe
        ///     characters e.g. &lt;TAB&gt;||&lt;ENTER&gt;
        /// </summary>
        SendKeys,
    ...
    public async override Task Process(LSGlobals ls)
    {
        ...
        if (browser == null && Action != BrowserAction.Open) throw new Exception("Open a browser first!");
        ...
        switch (Action)
        {
            ...
            case BrowserAction.SendKeys:
                ...
                foreach (var s in replacedInput.Split(new[] { "||" }, StringSplitOptions.None))
                    switch (s)
                    {
                        case "<TAB>":
                ...
                if (replacedInput.Contains("<ENTER>") ||
                    replacedInput.Contains("<BACKSPACE>")) // These might lead to a page change
                    UpdateSeleniumData(data);
                break;
                ...
                foreach (var cookie in data.COOKIES)
                    try
                    {
                        browser.Manage().Cookies
                            .AddCookie(new Cookie(cookie.Key, cookie.Value, baseURL, "/", DateTime.MaxValue));
                    }
                    catch
                    {
                    }
        ...
        data.Logger.Log(string.Format("Executed browser action {0} on input {1}", Action, replacedInput),
            LogColors.White);
    }
    ...
    public static void OpenBrowser(BotData data)
    {
        ...
        switch (provider.BrowserType)
        {
            case SeleniumBrowserType.Chrome:
                ...
                if (data.ConfigSettings.BrowserSettings.Headless) chromeop.AddArgument("--headless");

                // TODO: Readd support for chrome extensions

                if (data.ConfigSettings.BrowserSettings.DismissDialogs) chromeop.AddArgument("--disable-notifications");
                ...
                if (data.UseProxy)
                    ...
                    chromeop.AddArgument(
                        $"--proxy-server={data.Proxy.Type.ToString().ToLower()}://{data.Proxy.Host}:{data.Proxy.Port}");
            ...
            case SeleniumBrowserType.Firefox:
                ...
                if (Helpers.Utils.IsDocker()) fireop.AddArgument("--whitelisted-ips=''");

                if (data.ConfigSettings.BrowserSettings.Headless) fireop.AddArgument("--headless");
