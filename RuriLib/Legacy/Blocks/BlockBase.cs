...
public abstract class BlockBase
{
    ...
    public static List<string> ReplaceValuesRecursive(string original, LSGlobals ls)
    {
        ...
        foreach (Match m in matches)
        {
            ...
            if (variable is ListOfStringsVariable list) variables.Add(list);
        }
        ...
        if (match.Success)
        {
            ...
            var dict = GetVariables(data).Get<DictionaryOfStringsVariable>(name) ??
                       globals.Get<DictionaryOfStringsVariable>(name);
        ...
        return toReplace.Select(i => ReplaceValues(i, ls)).ToList();
    }
    ...
    public static Dictionary<string, string> ReplaceValues(Dictionary<string, string> original, LSGlobals ls)
    {
        ...
        foreach (var kvp in original) newDict[ReplaceValues(kvp.Key, ls)] = ReplaceValues(kvp.Value, ls);
    ...
    public static string ReplaceValues(string original, LSGlobals ls)
    {
        ...
        do
        {
            ...
            if (lastCaptchaInfo is not null) output = output.Replace("<CAPTCHAID>", lastCaptchaInfo.Id);

            // TODO: Readd this
            // output = output.Replace("<BOTNUM>", data.BotNumber.ToString());

            if (data.Proxy != null) output = output.Replace("<PROXY>", data.Proxy.ToString());
                ...
                switch (v)
                {
                    ...
                    case DictionaryOfStringsVariable:
                        ...
                        if (args.Contains("(") && args.Contains(")"))
                        {
                            ...
                            if (dict.ContainsKey(key)) output = output.Replace(full, dict[key]);
                        }
    ...
    protected static void InsertVariable(LSGlobals ls, bool isCapture, string value, string variableName,
        ...
        => InsertVariable(ls, isCapture, false, new[] { value }, variableName, prefix, suffix, urlEncode, createEmpty);
    ...
    /// </summary>
    protected static void InsertVariable(LSGlobals ls, bool isCapture, bool recursive, IEnumerable<string> values,
        string variableName,
        string prefix = "", string suffix = "", bool urlEncode = false, bool createEmpty = true)
    {
        var data = ls.BotData;
        ...
        if (urlEncode) list = list.Select(Uri.EscapeDataString).ToList();
        ...
        if (recursive)
        {
            if (list.Count > 0 || createEmpty)
                variable = new ListOfStringsVariable(list) { Name = variableName };
        }
        else
        {
            if (list.Count == 0)
            {
                if (createEmpty)
                    variable = new StringVariable(string.Empty) { Name = variableName };
            }
            else
                variable = new StringVariable(list.First()) { Name = variableName };
        }

        if (variable != null)
        {
            GetVariables(data).Set(variable);
            data.Logger.Log($"Parsed variable | Name: {variable.Name} | Value: {variable.AsString()}",
                isCapture ? LogColors.OrangeRed : LogColors.Gold);
