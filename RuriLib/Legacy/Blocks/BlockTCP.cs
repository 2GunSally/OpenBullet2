        ...
        switch (TCPCommand)
        {
            case TCPCommand.Connect:
                ...
                if (tcp.Connected)
                {
                    ...
                    data.Logger.Log($"Succesfully connected to host {h} on port {p}. The server says:",
                        LogColors.Green);
                ...
                {
                    GetVariables(data)
                        .Set(new StringVariable(response) { Name = VariableName, MarkedForCapture = IsCapture });
            ...
            case TCPCommand.Disconnect:
                if (tcp == null) throw new Exception("Make a connection first!");
            ...
            case TCPCommand.Send:
                ...
                if (WebSocket)
                {
                    ...
                    for (var i = 0; i < payload.Length; i++) bl.Add((byte)(payload[i] ^ mask[i % 4]));
