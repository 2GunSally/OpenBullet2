...
public class SBlockElementAction : BlockBase
{
    ...
    /// <summary>
    ///     Whether the GetText and GetAttribute actions should be executed on all the elements matched by the locator and
    ...
    public async override Task Process(LSGlobals ls)
    {
        ...
        if (browser == null) throw new Exception("Open a browser first!");
        ...
        if (Action != ElementAction.WaitForElement)
        {
            ...
            if (ElementIndex + 1 > elements.Count) throw new Exception("Cannot find the element on the page");
        }
        ...
        switch (Action)
        {
            ...
            case ElementAction.WaitForElement:
                ...
                if (!found) throw new TimeoutException("Timed out while waiting for the element");
        ...
        data.Logger.Log(string.Format("Executed action {0} on the element with input {1}", Action, replacedInput),
            LogColors.White);

        if (outputs.Count > 0) InsertVariable(ls, IsCapture, Recursive, outputs, OutputVariable);
    }
