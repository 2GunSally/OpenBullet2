...
public static class ConfigConverter
{
    public static Config Convert(string fileContent, string id)
    {
        ...
        var split = fileContent.Split(new[] { "[SETTINGS]", "[SCRIPT]" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        var newConfig = new Config { Id = id, Mode = ConfigMode.Legacy };
    ...
    private static void ApplyLegacySettings(Config newConfig, LegacyConfigSettings legacySettings)
    {
        ...
        if (legacySettings.ContinueOnCustom)
            newConfig.Settings.GeneralSettings.ContinueStatuses = new[] { "SUCCESS", "NONE", "CUSTOM" };

        // Data
        newConfig.Settings.DataSettings.AllowedWordlistTypes =
            new[] { legacySettings.AllowedWordlist1, legacySettings.AllowedWordlist2 };
        ...
        if (legacySettings.OnlySsl)
            newConfig.Settings.ProxySettings.AllowedProxyTypes = new[] { ProxyType.Http };
        else if (legacySettings.OnlySocks)
            newConfig.Settings.ProxySettings.AllowedProxyTypes =
                new[] { ProxyType.Socks4, ProxyType.Socks4a, ProxyType.Socks5 };
        newConfig.Settings.ProxySettings.BanLoopEvasion =
            Math.Max(0, legacySettings.BanLoopEvasionOverride); // -1 needs to be converted to 0
    ...
    private static DataRule BuildDataRule(LegacyDataRule oldRule)
    {
        ...
        switch (oldRule.RuleType)
        {
            case LegacyRuleType.MustMatchRegex:
                return new RegexDataRule { SliceName = oldRule.SliceName, RegexToMatch = oldRule.RuleString };
            ...
            case LegacyRuleType.MustContain:
                ...
                return new RegexDataRule { SliceName = oldRule.SliceName, RegexToMatch = sb.ToString() };
