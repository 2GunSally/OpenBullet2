        ...
        return () =>
        {
            switch (field)
            {
                ...
                case "USEPROXY":
                    ...
                    if (use == "TRUE")
                        data.UseProxy = true;
                    else if (use == "FALSE") data.UseProxy = false;
                    break;
                ...
                case "VAR":
                    ...
                    BlockBase.GetVariables(data).Set(new StringVariable(varValue) { Name = varName });
                    break;

                case "CAP":
                    var capName = LineParser.ParseLiteral(ref input, "NAME", true, ls);
                    var capValue = LineParser.ParseLiteral(ref input, "VALUE", true, ls);
                    BlockBase.GetVariables(data).Set(new StringVariable(capValue) { Name = capName });
                ...
                case "GVAR":
                    try
                    {
                        ...
                        ls.Globals.Set(new StringVariable(globalVarValue) { Name = globalVarName });
                    }
                    catch
                    {
                    }
                ...
                case "NEWGVAR":
                    try
                    {
                        var globalVarName = LineParser.ParseLiteral(ref input, "NAME", true, ls);
                        var globalVarValue = LineParser.ParseLiteral(ref input, "VALUE", true, ls);
                        ls.Globals.SetIfNew(new StringVariable(globalVarValue) { Name = globalVarName });
                    }
                    catch
                    {
                    }
                ...
                case "GCOOKIES":
                    ...
                    foreach (var cookie in data.COOKIES) ls.GlobalCookies.Add(cookie.Key, cookie.Value);
                    break;
