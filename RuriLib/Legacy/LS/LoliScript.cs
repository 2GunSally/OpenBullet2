...
public class LoliScript
{
        ...
        try
        {
            ...
            if (BlockParser.IsBlock(CurrentLine))
            {
                ...
                try
                {
                    ...
                    if (!block.Disabled) await block.Process(ls);
                }
                catch (Exception ex)
                {
            ...
            else
            {
                ...
                var token = LineParser.ParseToken(ref cfLine, TokenType.Parameter,
                    false); // This proceeds, so we have the cfLine ready for next parsing
                ...
                switch (token.ToUpper())
                {
                    case "IF":
                        ...
                        {
                            i = ScanFor(lines, i, true, new[] { "ENDIF", "ELSE" });
                    ...
                    case "ELSE":
                        ...
                        i = ScanFor(lines, i, true, new[] { "ENDIF" });
                        data.Logger.Log($"Jumping to line {i + 1}", LogColors.White);
                        break;
                    ...
                    case "WHILE":
                        ...
                        if (!ParseCheckCondition(ref cfLine, ls))
                        {
                            i = ScanFor(lines, i, true, new[] { "ENDWHILE" });
                            data.Logger.Log($"Jumping to line {i + 1}", LogColors.White);
                        }
                    ...
                    case "ENDWHILE":
                        ...
                        i = ScanFor(lines, i, false, new[] { "WHILE" }) - 1;
                        data.Logger.Log($"Jumping to line {i + 1}", LogColors.White);
                        break;
                    ...
                    case "JUMP":
                        ...
                        try
                        {
                            ...
                            i = ScanFor(lines, -1, true, new[] { $"{label}" }) - 1;
                    ...
                    case "BEGIN":
                        ...
                        {
                            case "SCRIPT":
                                language = (ScriptingLanguage)LineParser.ParseEnum(ref cfLine, "LANGUAGE",
                                    typeof(ScriptingLanguage));
                                var end = 0;
                                try
                                {
                                    end = ScanFor(lines, i, true, new[] { "END" }) - 1;
                                }
                                catch
                                {
                    ...
                    case "END":
                        ...
                        switch (endToken.ToUpper())
                        {
                            case "SCRIPT":
                                ...
                                {
                                    var errorMessage = data.Providers.GeneralSettings.VerboseMode
                                        ? ex.ToString()
                                        : ex.Message;
                                    data.Logger.Log($"The script failed to be executed: {errorMessage}",
                                        LogColors.Tomato);
                                }
    ...
    /// <param name="outputs">
    ///     The variables that should be extracted from the script's scope and set into the BotData local
    ///     variables
    /// </param>
    /// <param name="data">The BotData needed for variable replacement</param>
        ...
        try
        {
            switch (language)
            {
                case ScriptingLanguage.JavaScript:
                    ...
                    foreach (var name in outVarList)
                        try
                        {
                            ...
                            if (isArray)
                                variableList.Set(
                                    new ListOfStringsVariable(value.TryCast<List<string>>()) { Name = name });
                            else
                ...
                case ScriptingLanguage.IronPython:
                    ...
                    // Print other info
                    if (result != null) data.Logger.Log($"Completion value: {result}", LogColors.White);

                    break;
            }

            data.Logger.Log($"Execution completed in {(DateTime.Now - start).TotalSeconds} seconds",
    ...
    private string[] GetCompressedLines()
    {
        var i = 0;
        var isScript = false;
        var compressed = Script.Split(new[] { Environment.NewLine }, StringSplitOptions.None).ToList();
        while (i < compressed.Count - 1)
            if (!isScript && BlockParser.IsBlock(compressed[i]) &&
                (compressed[i + 1].StartsWith(" ") || compressed[i + 1].StartsWith("\t")))
            {
                compressed[i] += $" {compressed[i + 1].Trim()}";
                compressed.RemoveAt(i + 1);
            }
            else if (!isScript && BlockParser.IsBlock(compressed[i]) &&
                     (compressed[i + 1].StartsWith("! ") || compressed[i + 1].StartsWith("!\t")))
            {
    ...
    private static bool IsCustomStatus(string status)
        => !new[] { "SUCCESS", "FAIL", "RETRY", "BAN", "ERROR", "INVALID", "NONE" }.Contains(status);
    ...
    /// <summary>
    ///     Transforms the script into a list of blocks. The blocks that cannot be converted will be created as BlockLSCode
    ///     blocks.
    /// </summary>
    /// <returns></returns>
        ...
        foreach (var compressedLine in compressed.Where(c => !string.IsNullOrEmpty(c.Trim())))
            ...
            else
            {
                ...
                if (compressedLine.StartsWith("BEGIN SCRIPT"))
                    isScript = true;
                else if (compressedLine.StartsWith("END SCRIPT")) isScript = false;
            }
    ...
    public void FromBlocks(List<BlockBase> blocks)
    {
        ...
        foreach (var block in blocks) Script += block.ToLS() + Environment.NewLine + Environment.NewLine;
    }
