...
public static class LineParser
{
    ...
    public static string ParseToken(ref string input, TokenType type, bool essential, bool proceed = true)
    {
        ...
        if (m.Success)
        {
            ...
            if (proceed) input = input.Substring(token.Length).Trim();
        ...
        {
            if (essential) throw new ArgumentException("Cannot parse token");
        }
    ...
    {
        var result = ParseToken(ref input, TokenType.Parameter, true).Split('=');
        PropertyInfo prop;
    ...
    /// <summary>
    ///     Makes sure that a specified identifier is present and moves past it. An exception will be thrown if the identifier
    ///     is not present.
    /// </summary>
    /// <param name="input">The reference to the line of code</param>
