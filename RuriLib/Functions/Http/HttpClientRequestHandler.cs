...
internal class HttpClientRequestHandler : HttpRequestHandler
{
    public async override Task HttpRequestStandard(BotData data, StandardHttpRequestOptions options)
    {
        ...
        foreach (var header in options.CustomHeaders) request.Headers.TryAddWithoutValidation(header.Key, header.Value);
            ...
            switch (c)
            {
                ...
                case FileHttpContent x:
                    lock (FileLocker.GetHandle(x.FileName))
                    {
                        ...
                        var fileContent = CreateFileContent(fileStream, x.Name, Path.GetFileName(x.FileName),
                            x.ContentType);
    ...
    private static void LogHttpRequestData(BotData data, HttpRequestMessage request, string content = null,
        string boundary = null)
    {
        ...
        writer.WriteLine(
            $"{request.Method.Method} {request.RequestUri.PathAndQuery} HTTP/{request.Version.Major}.{request.Version.Minor}");
        ...
        CookieContainer cookieContainer, HttpRequestOptions requestOptions)
    {
        ...
        data.COOKIES.Clear();
        foreach (Cookie cookie in cookies) data.COOKIES[cookie.Name] = cookie.Value;
        ...
        if (requestOptions.DecodeHtml) data.SOURCE = WebUtility.HtmlDecode(data.SOURCE);
