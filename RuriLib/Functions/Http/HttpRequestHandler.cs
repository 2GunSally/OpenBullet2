...
internal abstract class HttpRequestHandler
{
    ...
    protected static StreamContent CreateFileContent(Stream stream, string fieldName, string fileName,
        string contentType)
    {
        ...
        foreach (var content in contents)
        {
            ...
            switch (content)
            {
                ...
                case FileHttpContent x:
                    writer.WriteLine(
                        $"Content-Disposition: form-data; name=\"{x.Name}\"; filename=\"{Path.GetFileName(x.FileName)}\"");
                    writer.WriteLine($"Content-Type: {GetMediaHeaderString(x.ContentType)}");
                    writer.WriteLine();
    ...
    {
        if (cipherSuites == null) return Array.Empty<TlsCipherSuite>();
