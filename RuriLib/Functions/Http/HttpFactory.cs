...
public class HttpFactory
{
    ...
    public static HttpClient GetHttpClient(Proxy proxy, HttpOptions options, CookieContainer cookieContainer)
    {
        ...
        return new HttpClient(handler) { Timeout = options.ReadWriteTimeout };
    }
    ...
    private static ProxyClient GetProxyClient(Proxy proxy, HttpOptions options)
    {
        ...
        else
        {
            ...
            if (proxy.NeedsAuthentication) settings.Credentials = new NetworkCredential(proxy.Username, proxy.Password);
        ...
        CookieContainer cookieContainer)
    {
        ...
        if (proxy == null)
            ...
            {
                case ProxyType.Http:
                    handler = new HttpClientHandler { Proxy = GetWebProxy(proxy) };
                    break;

                case ProxyType.Socks4:
                case ProxyType.Socks4a:
                case ProxyType.Socks5:
                    handler = new SocketsHttpHandler { Proxy = GetWebProxy(proxy) };
                    break;
    ...
    /// <summary>
    ///     Converts the <paramref name="protocol" /> to an SslProtocols enum. Multiple protocols are not supported and
    ///     SystemDefault is None.
    /// </summary>
