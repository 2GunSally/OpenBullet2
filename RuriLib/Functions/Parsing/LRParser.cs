...
{
    /// <summary>
    ///     Parses all strings between <paramref name="leftDelim" /> and <paramref name="rightDelim" /> in the
    ///     <paramref name="input" />.
    /// </summary>
    /// <param name="caseSensitive">Whether the case is important</param>
    public static IEnumerable<string> ParseBetween(string input, string leftDelim, string rightDelim,
        bool caseSensitive = true)
    {
        ...
        if ((leftDelim != string.Empty && !input.Contains(leftDelim, comp)) ||
            (rightDelim != string.Empty && !input.Contains(rightDelim, comp)))
            yield break;

        while ((leftDelim == string.Empty || input.Contains(leftDelim, comp)) &&
               (rightDelim == string.Empty || input.Contains(rightDelim, comp)))
        {
