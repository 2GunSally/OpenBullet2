...
public static class LoliCodeParser
{
    ...
    /// </summary>
    public static readonly string[] keyIdentifiers = {
        "BOOLKEY", "STRINGKEY", "INTKEY", "FLOATKEY", "LISTKEY", "DICTKEY"
    };
        ...
        T param) where T : BlockParameter
    {
        ...
        else if (input[0] == '$') // INTERPOLATED
        {
            ...
            setting.InterpolatedSetting = param switch {
                StringParameter x => new InterpolatedStringSetting {
                    Value = LineParser.ParseLiteral(ref input), MultiLine = x.MultiLine
                },
                ListOfStringsParameter _ => new InterpolatedListOfStringsSetting {
                    Value = LineParser.ParseList(ref input)
                },
                DictionaryOfStringsParameter _ => new InterpolatedDictionaryOfStringsSetting {
                    Value = LineParser.ParseDictionary(ref input)
                },
                _ => throw new NotSupportedException()
            };
            setting.FixedSetting = param switch // Initialize fixed setting as well, used for type switching
            {
                StringParameter x => new StringSetting {
                    Value = (setting.InterpolatedSetting as InterpolatedStringSetting).Value, MultiLine = x.MultiLine
                },
                ListOfStringsParameter _ => new ListOfStringsSetting {
                    Value = (setting.InterpolatedSetting as InterpolatedListOfStringsSetting).Value
                },
                DictionaryOfStringsParameter _ => new DictionaryOfStringsSetting {
                    Value = (setting.InterpolatedSetting as InterpolatedDictionaryOfStringsSetting).Value
                },
                _ => throw new NotSupportedException()
            };
        }
