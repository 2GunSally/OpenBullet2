...
public class DescriptorsRepository
{
    ...
    public Dictionary<string, BlockDescriptor> Descriptors { get; set; } = new();
            ...
            foreach (var method in methods)
            {
                ...
                Descriptors[method.Name] = new AutoBlockDescriptor {
                    ...
                    Images = method.GetCustomAttributes<BlockImage>()
                        .ToDictionary(a => a.id,
                            a => new BlockImageInfo {
                                Name = a.id.ToReadableName(), MaxWidth = a.maxWidth, MaxHeight = a.maxHeight
                            })
                };
            }
        }
    ...
    private static BlockParameter BuildBlockParameter(ParameterInfo info)
    {
        ...
        else if (interpParam != null) parameter.InputMode = SettingInputMode.Interpolated;
    ...
    private static BlockParameter ToBlockParameter(ParameterInfo parameter)
    {
        var dict = new Dictionary<Type, Func<BlockParameter>> {
            {
                ...
                typeof(int),
                () => new IntParameter { DefaultValue = parameter.HasDefaultValue ? (int)parameter.DefaultValue : 0 }
            }, {
                typeof(float),
                () => new FloatParameter {
        ...
        if (parameter.ParameterType.IsEnum)
        {
            ...
            if (blockParamAttribute != null) blockParam.AssignedName = blockParamAttribute.name;
