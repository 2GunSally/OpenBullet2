...
");
    ...
    private static void Run(Options opts)
    {
        ...
        if (string.IsNullOrEmpty(opts.WordlistFile) && !string.IsNullOrEmpty(opts.WordlistRange))
        {
            ...
            dataPool = new RangeDataPool(
                Convert.ToInt64(splitRange[0]),
                Convert.ToInt32(splitRange[1]),
                splitRange.Length > 2 ? Convert.ToInt32(splitRange[2]) : default,
                splitRange.Length > 3 ? Convert.ToBoolean(splitRange[3]) : default,
                opts.WordlistType
            );
        }
        else
        ...
        job = new MultiRunJob(rlSettings, pluginRepo) {
            ...
            ProxySources =
                new List<ProxySource> { new FileProxySource(opts.ProxyFile) { DefaultType = opts.ProxyType } },
    ...
    private class Options
    {
        ...
        [Option("wordlist-range", Required = false,
            HelpText =
                "Wordlist Range to be processed, syntax: start,amount,step,pad(True or False) . Only start and amount are necessary.")]
        public string WordlistRange { get; }

        [Option("wltype", Required = true,
            HelpText = "Type of the wordlist loaded (see Environment.ini for all allowed types).")]
        ...
        [Option('b', "bots", Default = 0,
            HelpText = "Number of concurrent bots working. If not specified, the config default will be used.")]
