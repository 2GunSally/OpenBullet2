...
public class TaskBasedParallelizer<TInput, TOutput> : Parallelizer<TInput, TOutput>
{
    ...
    /// <inheritdoc />
    public TaskBasedParallelizer(IEnumerable<TInput> workItems,
    ...
    private async void Run()
    {
        ...
        while (queue.Count < BatchSize && items.MoveNext()) queue.Enqueue(items.Current);
        ...
        try
        {
            ...
            while (Progress < 1 && !hardCTS.IsCancellationRequested) await Task.Delay(100).ConfigureAwait(false);
        }
    ...
    public async override Task ChangeDegreeOfParallelism(int newValue)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < degreeOfParallelism - newValue; ++i) await semaphore.WaitAsync().ConfigureAwait(false);
            dopDecreaseRequested = false;
        }
