...
public partial class ProxyGroups
{
    ...
    protected async override Task OnParametersSetAsync()
    {
        ...
        gridColumns = c =>
        {
            ...
            c.Add(p => p.LastChecked).Titled(Loc["LastChecked"]).SetFilterWidgetType("DateTimeLocal")
        ...
        var client = new GridClient<ProxyEntity>(q => GetGridRows(gridColumns, q), query, false, "proxiesGrid",
                gridColumns, CultureInfo.CurrentCulture)
            .Sortable()
            .Filterable()
            .WithMultipleFilters()
            .SetKeyboard(true)
    ...
    private async Task ImportProxies()
    {
        ...
        if (!result.Cancelled)
        {
            ...
            await js.AlertSuccess(Loc["Imported"],
                $"{Loc["ProxiesImportedSuccessfully"]}: {dto.Lines.Distinct().Count()}");
        }
    }
    ...
    private async Task ExportProxies()
    {
        ...
        var outputBytes = Encoding.UTF8.GetBytes(outputProxies);
    ...
    private async Task MoveFiltered()
    {
        ...
        if (!result.Cancelled)
        {
            ...
            foreach (var proxy in filtered) proxy.Group = group;
