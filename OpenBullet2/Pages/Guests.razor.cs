...
public partial class Guests
{
    ...
    protected async override Task OnParametersSetAsync()
    {
        ...
        Action<IGridColumnCollection<GuestEntity>> columns = c =>
        {
            ...
            c.Add(g => g.AccessExpiration).Titled(Loc["AccessExpiration"]).SetFilterWidgetType("DateTimeLocal")
        ...
        var client = new GridClient<GuestEntity>(q => GetGridRows(columns, q), query, false, "guestsGrid", columns,
    ...
    {
        if (item.GetType() == typeof(GuestEntity)) selectedGuest = (GuestEntity)item;
    }
    ...
    private async Task DeleteGuest()
    {
        ...
        if (await js.Confirm(Loc["AreYouSure"], $"{Loc["ReallyDelete"]} {selectedGuest.Username}?", Loc["Cancel"]))
            ...
            if (await js.Confirm(Loc["DeleteEverything"], Loc["DeleteEverythingMessage"], Loc["Cancel"]))
            {
                ...
                foreach (var w in wordlistsToDelete) await WordlistRepo.Delete(w);
