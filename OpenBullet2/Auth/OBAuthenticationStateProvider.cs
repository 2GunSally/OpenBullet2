...
public class OBAuthenticationStateProvider : AuthenticationStateProvider
{
    ...
    public async override Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        try
        {
            var authenticatedUser = jwtValidator.ValidateToken(jwt, out var validatedToken);
        ...
        catch
        {
            ...
            var anonymousUser = new ClaimsPrincipal(new ClaimsIdentity(
                new[] { new Claim(ClaimTypes.Name, "anonymous"), new Claim(ClaimTypes.Role, "Anonymous") },
                "anonymous"));
    ...
    public async Task<int> GetCurrentUserId()
    {
        ...
        if (role == "Guest")
        {
            ...
            if (guest is not null) return guest.Id;
        }
    ...
    private async Task AuthenticateGuest(string username, string password, IPAddress ip)
    {
        ...
        if (entity == null) throw new EntryNotFoundException("Could not find a guest with the given username");
        ...
        var isValid = await Firewall.CheckIpValidity(ip,
        ...
        var claims = new[] {
            new Claim(ClaimTypes.Name, username), new Claim(ClaimTypes.Role, "Guest"),
