...
public partial class BlockInputField
{
    ...
    private Task<IEnumerable<string>> GetSuggestions(string partial)
    {
        ...
        foreach (var block in stack)
        {
            ...
            if (block.Settings.Any(s => s.Value == BlockSetting)) break;
    ...
    private IEnumerable<string> GetOutputVariables(BlockInstance block)
        => block switch {
            AutoBlockInstance x => x.Descriptor.ReturnType == null ? Array.Empty<string>() : new[] { x.OutputVariable },
            ParseBlockInstance x => new[] { x.OutputVariable },
