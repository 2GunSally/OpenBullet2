...
public partial class MultiRunJobViewer : IDisposable
{
    ...
    {
        if (details.Result.BotData.STATUS == "SUCCESS" &&
            OBSettingsService.Settings.CustomizationSettings.PlaySoundOnHit) _ = js.InvokeVoidAsync("playHitSound");
    }
    ...
    private void LogCompleted(object sender, EventArgs e) => Logger.LogInfo(Job.Id, Loc["TaskManagerCompleted"]);
    ...
    private async Task ShowFullLog()
    {
        ...
        {
            var errorMessage = lastSelectedHit.Config.Mode == ConfigMode.DLL
                ? Loc["BotLogCompiledConfigError"]
                : Loc["BotLogDisabledError"];
