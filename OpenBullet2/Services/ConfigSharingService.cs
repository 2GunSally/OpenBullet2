...
public class ConfigSharingService
{
    ...
    public void Save() => File.WriteAllText(EndpointsFile, JsonConvert.SerializeObject(Endpoints, jsonSettings));
        ...
        using (var archive = new ZipArchive(ms, ZipArchiveMode.Create, true))
            foreach (var configId in endpoint.ConfigIds)
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine(
                        $"Error while packing config {configId} for endpoint {endpoint.Route}: {ex.Message}");
                }
