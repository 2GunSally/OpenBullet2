...
public class Socks5ProxyClient : ProxyClient
{
    ...
    private async Task NegotiateAsync(NetworkStream nStream, CancellationToken cancellationToken = default)
    {
        ...
        else if (reply != CommandReplySucceeded) HandleCommandError(reply);
    }

    private async Task SendUsernameAndPasswordAsync(NetworkStream nStream,
        CancellationToken cancellationToken = default)
    {
        ...
        if (reply != CommandReplySucceeded) throw new ProxyException("Unable to authenticate proxy-server");
    }

    private static async Task RequestConnectionAsync(NetworkStream nStream, byte command, string destinationHost,
        int destinationPort,
        CancellationToken cancellationToken = default)
    {
        ...
        var reply = response[1];
        if (reply != CommandReplySucceeded) HandleCommandError(reply);
    }

    private static byte GetAddressType(string host)
    {
        if (!IPAddress.TryParse(host, out var ipAddr)) return AddressTypeDomainName;
