...
public class Socks4ProxyClient : ProxyClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await RequestConnectionAsync(client.GetStream(), CommandConnect, destinationHost, destinationPort,
                    cancellationToken)
    ...
    /// </summary>
    protected async virtual Task RequestConnectionAsync(NetworkStream nStream, byte command, string destinationHost,
        int destinationPort,
        CancellationToken cancellationToken = default)
    {
        ...
        if (reply != CommandReplyRequestGranted) HandleCommandError(reply);
    }
