...
public class HttpProxyClient : ProxyClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        commandBuilder.AppendFormat("CONNECT {0}:{1} HTTP/{2}\r\n{3}\r\n", destinationHost, destinationPort,
            ProtocolVersion, GenerateAuthorizationHeader());
    ...
    {
        if (Settings.Credentials == null || string.IsNullOrEmpty(Settings.Credentials.UserName)) return string.Empty;
    ...
    private static async Task<HttpStatusCode> ReceiveResponseAsync(NetworkStream nStream,
        CancellationToken cancellationToken = default)
    {
