...
public class HttpRequest : IDisposable
{
    ...
    public async Task<byte[]> GetBytesAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (Content != null) ms.Write(await Content.ReadAsByteArrayAsync(cancellationToken));
    ...
    private string BuildHeaders()
    {
        ...
        if (!HeaderExists("Host", out _)) finalHeaders.Add("Host", Uri.Host);

        // If there is no Connection header, add it
        if (!HeaderExists("Connection", out var connectionHeaderName)) finalHeaders.Add("Connection", "Close");

        // Add the non-content headers
        foreach (var header in Headers) finalHeaders.Add(header);
