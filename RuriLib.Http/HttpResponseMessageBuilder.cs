...
internal class HttpResponseMessageBuilder
{
    ...
    private bool ReadHeadersFastPath(ref ReadOnlySequence<byte> buff)
    {
        ...
        if ((endofheadersindex = buff.FirstSpan.IndexOf(CRLFCRLF_Bytes)) > -1)
        {
            ...
            foreach (var Line in Lines) ProcessHeaderLine(Line);

            buff = buff.Slice(endofheadersindex +
    ...
    private bool ReadHeadersSlowerPath(ref ReadOnlySequence<byte> buff)
    {
        ...
        while (reader.TryReadTo(out ReadOnlySpan<byte> Line, CRLF))
        {
    ...
    private void ProcessHeaderLine(ReadOnlySpan<byte> header)
    {
        ...
        var headerValue = headerValuespan[0] == (byte)' '
            ? Encoding.UTF8.GetString(headerValuespan.Slice(1))
            : Encoding.UTF8.GetString(headerValuespan); // trim the wight space
        ...
        else if (ContentHelper.IsContentHeader(headerName))
        {
            ...
            {
                values = new List<string> { headerValue };
    ...
    private async Task ReceiveContentAsync(bool readResponseContent = true,
        CancellationToken cancellationToken = default)
    {
        ...
        if (contentHeaders.Count != 0)
        {
            ...
            foreach (var pair in contentHeaders) response.Content.Headers.TryAddWithoutValidation(pair.Key, pair.Value);
        }
    }
    ...
    private async Task<Stream> GetResponcestreamUntilClose(CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (res.IsCanceled) cancellationToken.ThrowIfCancellationRequested();
            var buff = res.Buffer;
            ...
            if (res.IsCompleted ||
                res.Buffer.Length ==
                0) // here the pipe will be complete if the server closes the connection or sends 0 length byte array
                break;
        }
    ...
    private async Task<Stream> ReciveContentLength(CancellationToken cancellationToken)
    {
        ...
        if (contentLength == 0) return contentlenghtStream;
        ...
        while (true)
        {
            ...
            if (contentlenghtStream.Length >= contentLength) return contentlenghtStream;
    ...
    private string GetContentEncoding()
    {
        ...
        if (contentHeaders.TryGetValue("Content-Encoding", out var values)) encoding = values[0];
    ...
    private async Task<Stream> ReceiveMessageBodyChunked(CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (chunkedDecoder.Finished) return chunkedDecoder.DecodedStream;
            if (res.IsCanceled || res.IsCompleted)
            {
                reader.Complete();
                cancellationToken.ThrowIfCancellationRequested();
