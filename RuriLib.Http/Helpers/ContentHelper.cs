...
static internal class ContentHelper
{
    ...
    private static readonly string[] contentHeaders = {
        "Last-Modified", "Expires", "Content-Type", "Content-Range", "Content-MD5", "Content-Location",
        "Content-Length", "Content-Language", "Content-Encoding", "Allow"
    };

    public static bool IsContentHeader(string name) =>
        contentHeaders.Any(h => h.Equals(name, StringComparison.OrdinalIgnoreCase));
}