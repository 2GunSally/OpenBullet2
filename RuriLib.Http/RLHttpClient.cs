...
public class RLHttpClient : IDisposable
{
    ...
    {
        ProxyClient = proxyClient ?? new NoProxyClient();
    }
    ...
    {
        if (redirects > MaxNumberOfRedirects) throw new Exception("Maximum number of redirects exceeded");
        ...
        try
        {
            ...
            if ((int)responseMessage.StatusCode / 100 == 3 && AllowAutoRedirect)
            {
                ...
                if (locationHeaderName is null)
                    throw new Exception(
                        $"Status code was {(int)responseMessage.StatusCode} but no Location header received. " +
                        $"Disable auto redirect and try again.");

                Uri.TryCreate(responseMessage.Headers[locationHeaderName], UriKind.RelativeOrAbsolute,
                    out var newLocation);
                ...
                if (request.Uri.Host != redirectUri.Host)
                {
                    ...
                    if (request.HeaderExists("Host", out var hostHeaderName)) request.Headers.Remove(hostHeaderName);
    ...
    private async Task SendDataAsync(HttpRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await connectionCommonStream.WriteAsync(buffer.AsMemory(0, buffer.Length), cancellationToken)
    ...
    private Task<HttpResponse> ReceiveDataAsync(HttpRequest request,
        ...
        new HttpResponseBuilder().GetResponseAsync(request, connectionCommonStream, ReadResponseContent,
            cancellationToken);
    ...
    private async Task CreateConnection(HttpRequest request, CancellationToken cancellationToken)
    {
        ...
        if (uri.Scheme.Equals("https", StringComparison.OrdinalIgnoreCase))
            try
            {
                ...
                if (UseCustomCipherSuites) sslOptions.CipherSuitesPolicy = new CipherSuitesPolicy(AllowedCipherSuites);
            ...
            {
                if (ex is IOException || ex is AuthenticationException) throw new ProxyException("Failed SSL connect");
    ...
    public TlsCipherSuite[] AllowedCipherSuites { get; set; } = {
        TlsCipherSuite.TLS_AES_128_GCM_SHA256, TlsCipherSuite.TLS_CHACHA20_POLY1305_SHA256,
        TlsCipherSuite.TLS_AES_256_GCM_SHA384, TlsCipherSuite.TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,
        TlsCipherSuite.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
        TlsCipherSuite.TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256,
        TlsCipherSuite.TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256,
        TlsCipherSuite.TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384, TlsCipherSuite.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384,
        TlsCipherSuite.TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA, TlsCipherSuite.TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA,
        TlsCipherSuite.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA, TlsCipherSuite.TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA,
        TlsCipherSuite.TLS_RSA_WITH_AES_128_GCM_SHA256, TlsCipherSuite.TLS_RSA_WITH_AES_256_GCM_SHA384,
