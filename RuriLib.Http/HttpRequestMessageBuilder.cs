...
static internal class HttpRequestMessageBuilder
{
    ...
    public static string BuildHeaders(HttpRequestMessage request, CookieContainer cookies = null)
    {
        ...
        if (string.IsNullOrEmpty(request.Headers.Host)) headers.Add("Host", request.RequestUri.Host);

        // Add the Connection: Close header if none is present
        if (request.Headers.Connection.Count == 0) headers.Add("Connection", "Close");

        // Add the non-content headers
        foreach (var header in request.Headers) headers.Add(header.Key, GetHeaderValue(header));
        ...
        if (cookies != null)
        {
            ...
            if (cookiesCollection.Count > 0)
            {
                ...
                if (cookieBuilder.Length > 2) cookieBuilder.Remove(cookieBuilder.Length - 2, 2);
        ...
        {
            foreach (var header in request.Content.Headers) headers.Add(header.Key, GetHeaderValue(header));
            ...
            if (!headers.Any(h => h.Key.Equals("Content-Length", StringComparison.OrdinalIgnoreCase)))
            {
                ...
                if (contentLength.HasValue && contentLength.Value > 0) headers.Add("Content-Length", contentLength);
            }
        }
