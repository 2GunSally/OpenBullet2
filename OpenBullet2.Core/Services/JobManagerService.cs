...
public class JobManagerService : IDisposable
{
    ...
    {
        if (sender is not MultiRunJob job || job.DataPool is not WordlistDataPool pool) return;
        ...
        try
        {
            ...
            if (record == null)
                await recordRepo.Add(new RecordEntity {
                    ConfigId = job.Config.Id, WordlistId = pool.Wordlist.Id, Checkpoint = checkpoint
                });
            else
            {
    ...
    {
        if (sender is not MultiRunJob job) return;
