...
public class JobFactoryService
{
    ...
    public JobFactoryService(ConfigService configService, RuriLibSettingsService settingsService,
        ...
        HitStorageService hitStorage, ProxySourceFactoryService proxySourceFactory,
        DataPoolFactoryService dataPoolFactory,
        ProxyReloadService proxyReloadService, IRandomUAProvider randomUAProvider, IRNGProvider rngProvider,
        IJobLogger logger,
        IProxyRepository proxyRepo, IConfiguration config)
    {
        ...
        if (botLimit is not null) BotLimit = int.Parse(botLimit);
    }
    ...
    private MultiRunJob MakeMultiRunJob(MultiRunJobOptions options)
    {
        ...
        var job = new MultiRunJob(settingsService, pluginRepo, logger) {
            ...
            Providers = new Providers(settingsService) {
