...
{
    private readonly Dictionary<UAPlatform, UserAgent[]> distributions = new();
    ...
    private static bool BelongsToPlatform(UAPlatform current, UAPlatform required) => required switch {
        ...
        UAPlatform.Mobile => current == UAPlatform.iPhone || current == UAPlatform.iPad ||
                             current == UAPlatform.Android,
        _ => current == required
    };
}