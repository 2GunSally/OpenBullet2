...
public class DataPoolFactoryService
{
    ...
    private async Task<DataPool> MakeWordlistDataPool(WordlistDataPoolOptions options)
    {
        ...
        if (entity == null) throw new Exception($"Wordlist entity not found: {options.WordlistId}");

        if (!File.Exists(entity.FileName)) throw new Exception($"Wordlist file not found: {entity.FileName}");
